            ...
            foreach (var rec in records.Distinct())
            {
                ...
                var code = ContainingTypesBuilder.Build(symbol, content =>
                {
                    foreach (var propertySymbol in symbol.GetProperties())
                    {
                        ...
                        content.AppendLine($"public static global::Tinkoff.Visor.ILens<{symbol.ToDisplayString()}, {propertyType}> {propertyName}Lens =>");
                        content.AppendLine($"global::Tinkoff.Visor.Lens<{symbol.ToDisplayString()}, {propertyType}>.New(");
                        content.AppendLine($"p => p.{propertyName},");
                        content.AppendLine($"f => p => p with {{{propertyName} = f(p.{propertyName})}}");
                        content.AppendLine(");");
